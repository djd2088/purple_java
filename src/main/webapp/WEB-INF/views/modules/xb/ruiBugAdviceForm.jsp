...
<body>
	...
	<form:form id="inputForm" modelAttribute="ruiBugAdvice" action="${ctx}/xb/ruiBugAdvice/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="user" name="user.id" value="${ruiBugAdvice.user.id}" labelName="user.name" labelValue="${ruiBugAdvice.user.name}"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="createTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${ruiBugAdvice.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">is_deal：</label>
			<div class="controls">
				<form:input path="isDeal" htmlEscape="false" maxlength="4" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="xb:ruiBugAdvice:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
